...
function Dashboard() {
  ...
  const { setopn_menu, Grid_Dis, setGrid_Dis, filter_prd, category, handleCategory, handleBrand, selectedBrands } = useContext(App_context)
  const navigate = useNavigate()

  return (
    <div className='w-full min-h-screen bg-[whiteSmoke] ' onClick={() => setopn_menu("")}>
      <Navbar />
      <div className='w-full min-h-screen flex flex-wrap  '>
            ...
            <ul className='flex items-center gap-3 w-full h-max mt-5 flex-wrap'>
              {
                ["all", "computer", "Gaming Furniture", "groceries", "electronics", "fragrances", "furniture", "fitness", "kitchen", "footwear", "beauty",
                  "supplements", "lifestyle", "health", "home decor", "travel", "books", "accessories", "home"].map((cat, i) => {
                    return (
                      <li key={i} className={`text-sm py-[2px] px-1 rounded-lg  cursor-pointer ${category == cat ? "bg-black text-white " : "hover:scale-103 hover:bg-black/20 bg-black/10"} transition-all duration-300 ease-in-out  gx_sh text-black capitalize `} onClick={() => handleCategory(cat)}  >{cat}</li>
                    )
                  })
              }
            </ul>
            <p className='text-black text-2xl font-bold flex mt-5 items-center tb_sh gap-3'> <TbBrandBunpo className='text-cyan-500' />Brand</p>
            <div className=' w-full h-max mt-5 px-3'>
              {
                ["Logitech", "RazerMax", "FreshFarm", "FarmFresh", "CitrusFresh", "Samsung", "Chanel", "LuxHome", "Tom Ford", "MediterraneanGold", "OrchardFresh", "SteelSeries", "Sony",
                  "TechPro", "ZenFit", "BrewMaster", "Nike", "GlowSkin", "FitNutrition", "WoodCraft", "ChargeTech", "LuxVision", "TeaGarden", "ViewMax", "StylePro", "HydroFlask", "SoundWave", "ComfortDesk",
                  "Italiana", "LeatherCraft", "OralTech", "ZenScents", "DeskPro", "SkinGlow", "AdventurePack", "CookSmart", "ClassicReads", "FitTracker", "GreenThumb", "FitBands", "ArtStudio", "SleepComfort"].map((b, d) => {
                    return (
                      <label key={d} htmlFor="" className='flex items-center gap-3 text-lg font-normal text-black/70 '>
                        <input type="checkbox" checked={selectedBrands.includes(b)} onChange={() => handleBrand(b)} name="" id="" />
                        <p>{b}</p>
                      </label>
                    )
                  })
        ...
        {/* ================SIDE BAR ========================== SIDE BAR ===================== */}
        <div className='w-[80%] min-h-screen max-h-[280vh] pt-[70px] overflow-y-auto'>
          <BackgroundPaths
            svgOptions={{ duration: 8 }} className=' bg-transparent w-full h-auto '
          >
            ...
            <div className='w-full  relative  py-7 px-5 flex  items-center justify-evenly flex-wrap '>
              {/* /////////////////////////////////// CARD  */}
